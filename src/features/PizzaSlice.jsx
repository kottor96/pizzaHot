...
const PizzaSlice = createSlice({
    ...
    reducers:{
        ajouterPanier(state,action){
            const pizza = state.listPizza.find((el)=>el.nom===action.payload)
        ...
        switchIngredient(state,action){
            const pizzaBis = state.panier[action.payload.index]
            const ingredient = pizzaBis[action.payload.index]
